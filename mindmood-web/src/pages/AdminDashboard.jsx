...
export default function AdminDashboard() {
  ...
  const handleTriaje = async (item) => {
    ...
    try {
      await supabase.rpc("admin_update_entry_status", { target_entry_id: item.id || item.entry_id, new_status: nextStatus });
      fetchAdminData();
    } catch (err) { if (import.meta.env.DEV) console.error(err); }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={Users} label="Total Users" value={stats?.total_users || 0} delta={`${stats?.total_users || 0} usuarios`} color="text-indigo-500" />
        <StatCard icon={Database} label="Data Points" value={totalEntries} delta={`${totalEntries} registros`} color="text-fuchsia-500" />
        ...
        <section className="lg:col-span-5 flex flex-col gap-8">
          <div className={`flex-1 p-8 relative overflow-hidden ${CARD}`}>
            ...
            <div className="h-64 relative">
              <ResponsiveContainer width="100%" height="100%">
                <RePieChart>
                  <Pie data={stabilityData} cx="50%" cy="50%" innerRadius={70} outerRadius={95} paddingAngle={10} dataKey="value" stroke="none">
                    {stabilityData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                  </Pie>
                </RePieChart>
              </ResponsiveContainer>
          ...
          {crisisCount > 0 && (
            <div className="p-8 bg-gradient-to-br from-indigo-600 to-fuchsia-600 rounded-[3rem] shadow-xl text-white">
              ...
              <button onClick={() => setSearchQuery("")} className="w-full py-4 bg-white/20 backdrop-blur-md rounded-2xl font-black hover:bg-white/30 transition-all">Desplegar Unidades</button>
            </div>
          )}
        </section>
      </div>
      ...
      <section className="space-y-8 mt-12 px-2">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="relative group">
            ...
            <input type="text" placeholder="Filtro rápido..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  ...
                  <div className="flex items-center gap-6 flex-1">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className="font-black text-base md:text-lg dark:text-white truncate max-w-[200px]">{item.student_email || item.email}</span>
                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-lg text-[8px] font-black tracking-widest uppercase">UID: {uid}</span>
                      </div>
                      <p className="text-slate-500 dark:text-slate-400 text-sm font-medium italic line-clamp-2">"{item.diary_text}"</p>
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-[11px] font-black" style={{ color: stColor }}>{stLabel}</span>
                        {hasContact && !contactRejected && <span className="text-[11px] font-black text-emerald-500">✓ Contactado</span>}
                        {contactRejected && <span className="text-[11px] font-black text-red-500">✗ Rechazado</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    {createdDate && (
                      <div className="text-right mr-4">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          {createdDate.toLocaleDateString("es", { day: "2-digit", month: "short" })}
                        </p>
                        <p className="font-black text-sm dark:text-white">{createdDate.toLocaleTimeString("es", { hour: "2-digit", minute: "2-digit" })}</p>
                      </div>
                    )}
                    ...
                    {!hasContact && st !== "resolved" && (
                      <button onClick={() => handleContact(item.user_id, item.id || item.entry_id)}
                        className="px-4 py-3 rounded-2xl font-black text-xs uppercase tracking-widest bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500/20 transition-all">
                        Contactar
                      </button>
                    )}
                  </div>
